...
- ESLintのルールはロジック部分のみ書く
    ...
    - プラグインの仕組み: [JavaScript Plugin Architectureというプラグイン設計について学ぶ無料の電子書籍を書いた](https://efcl.info/2016/06/06/javascript-plugin-architecture/)
...
- ASTの変換は結構泥臭い
- Babelは`path`オブジェクトに便利メソッドをたくさん持っている
    - `path.remove`でそのNodeを削除できる
...
- 削除だけではなく別のものへの置換も簡単
- `path.remove`ではなく`path.replaceWith(node, arguments)`を使う
...
- Babelはビルド向け、jscodeshiftはgrep+sedなどような大量のファイル変更向け
- [jscodeshift](https://github.com/facebook/jscodeshift)も内部的にBabelのパーサを使う
    - JavaScriptやTypeScriptを処理できるため
...
- jQueryやDOM APIようなコレクションAPIでASTを操作していく
- ESLint, Babel: トラバースの待ち受けを処理を書く(トップダウン)
- jscodeshift: 変換したいNodeを探して処理を書く(ボトムアップ)
